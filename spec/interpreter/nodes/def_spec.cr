...
describe "Interpreter - Def" do
  ...
  it_interprets %q(def foo; end)  { |itr| [TFunctor.new(itr.current_scope, [Def.new("foo")])] }
  it_interprets %q(
    def foo
      []
    end
  )                               { |itr| [TFunctor.new(itr.current_scope, [Def.new("foo", body: e(ListLiteral.new))])] }
  it_interprets %q(
    def foo
      true
      nil
  ...
  )                               { |itr| [TFunctor.new(itr.current_scope, [Def.new("foo", body: e(l(true), l(nil), l(1)))])] }

  it_interprets %q(
    def foo(a)
      a
    end
  )                               { |itr| [TFunctor.new(itr.current_scope, [Def.new("foo", [p("a")], body: e(Var.new("a")))])] }

  # Redefinition of a function is allowed. It will simply create a second clause.
  it_interprets %q(
    def foo; end
    def foo; end
  )                               { |itr| [TFunctor.new(itr.current_scope, [Def.new("foo"), Def.new("foo")])] }

  it_interprets %q(
    def foo(a); end
    def foo(b); end
  )                               { |itr| [TFunctor.new(itr.current_scope, [Def.new("foo", [p("a")]), Def.new("foo", [p("b")])])] }

  # Functions with different names should not be merged into one functor.
  it_interprets %q(
    def a; end
    def b; end
  )                               { |itr| [TFunctor.new(itr.current_scope, [Def.new("b")])] }
